...
### Roofline / MFU 定量分析（瓶颈的物理定性）
为什么 batch 增大先有用、mbsz≥4 后失效？用 GEMM 的 **roofline 模型**解释：

- GEMM `C[M,N] = A[M,K]×B[K,N]`，其中 **M ≈ token 数 = micro_bsz × seq_len**。
- **带宽受限区（斜坡）**：M 小时，权重 B 从 HBM 反复加载、复用不足，算术强度（FLOP/字节）低，
  时间被 HBM 带宽卡住。增大 mbsz → M 变大 → 每加载一次权重摊到更多 token → 算术强度上升 → 提速。
  这就是 mbsz 2→4 拿到 +33% 的机制。
- **计算受限区（平台）**：M 足够大（mbsz=4 时 M=16384）越过 roofline 脊点后，权重加载已被充分摊薄，
  时间几乎纯由矩阵核心 FLOPs 决定。**再加 batch = 成比例加 FLOPs = 成比例加时间**，
  tokens/s = FLOPs/时间 → 恒定。这正是 mbsz=6 仅 +1.3% 的原因（见 §7.5 扫描）。

**MFU 估算（当前 251.5K tok/s）**：
- 每 token 训练 FLOPs ≈ 6N = 6 × 7.6e9 ≈ 4.6e10。
- 总达成算力 = 251.5e3 × 4.6e10 ≈ 11.5 PFLOP/s，÷32 GPU ≈ **~358 TFLOP/s per GPU**。
- MI300X dense bf16 峰值 ~650–990 TFLOP/s → **MFU ≈ 40–55%**。

**40–55% MFU 是健康且接近实践上限的计算受限状态。** 距硬件峰值的剩余差距**不是 batch 能补的**，
来自三处非-GEMM 稀释：(1) 非 matmul 算子（RMSNorm/RoPE/softmax/逐元素/loss，不走矩阵核心，
compile 已融合部分）；(2) hipBLASLt/rocBLAS bf16 GEMM kernel 本身达不到理论 100%；(3) FSDP2
all-gather 未 100% 重叠的残余气泡。

**因此，唯一能再抬「计算受限区天花板」的物理途径是 FP8**（把矩阵核心峰值本身翻倍），而非任何调参——
这也正是 §7 把 FP8 列为「方向正确、被 ROCm hipBLASLt 卡住、等 ROCm 7.x」的深层原因。当前已把 MI300X
的 bf16 算力用到实践饱和点。
