...
>
> **📌 文档状态（2026-07 更新）**：§1–§8 为**设计计划稿**（部分数值已被后续实际落地覆盖，
> 相关处已加醒目注记并指向最终值）；**§9（tokenize 实际执行）与 §10（1T 训练落地）为实际
> 生效内容，配比/超参/吞吐一律以 §9.1 / §10 为准**。当前状态：**1T 正式训练已启动**
> （8 节点/64×MI300、fp8、~610K tok/s、238K steps、可断点续训）。

---

## 1. 目标与预算

> **已更新（见 §10 实际落地）**：平台从 32×MI300 迁到 **64×MI300（8 节点）**，
> fp8 默认 ON，实际吞吐 **~610K tok/s**，全局 batch 4.19M/step。下表为最终实际值。
...
| 全局 batch | mbsz4 × grad_accum4 × world64 × seq4096 = **4.19M tok/step** |
| 训练步数 | 1T / 4.19M ≈ **238K steps** |
| 吞吐 | fp8 + 64×MI300 实测 **~610K tok/s**（smoke 601–607K，正式 607–612K）|
| 训练耗时 | ≈ **~456 GPU-小时 ≈ ~19 天** wall-clock（可断点续训分段）|

---

## 2. 数据配比（早期计划稿 —— 已被 §10.1 最终落地取代）

> ⚠️ **本节为早期计划配比，已过时。最终实际配比见 §9.1 与 §10.1（plan B，8 源）。**
> 主要差异：code 源最终用 **`bigcode/starcoderdata`**（已获 HF token，非 codeparrot）
> 占 15%；math 拆为 finemath + infimath + open-web-math 三源；实际各源 token 见 §10.1。
> 下表保留作设计演进记录。

> **历史注记（已被 §9.1/§10.1 取代）**：本表及下方门控提示为**中间设计稿**。早期集群
> 无 HF token，一度改用非门控的 `codeparrot/codeparrot-clean`（仅 Python）并把 code 降到
> 4%。**最终已获 HF token 授权**，改回 `bigcode/starcoderdata` 并把 code 提到 **15%**，
> math 拆为 finemath+infimath+open-web-math 三源。**实际训练配比以 §9.1/§10.1 plan B 为准**，
> 下表仅存档演进。
...
### 数据加载器改造（`src/data.py`）
新增 **多源加权 IterableDataset**（最终实现为 **`EpochMixtureDataset`**，OLMo 式；
早期原型 `WeightedMultiSourceDataset` 保留兼容，见 §10.2）：
...
### 配比配置
命名配比定义在 `src/data_mix.py`（**最终实现见 §9.1 / §10.1，8 源 plan B**）。
train.py 加 `--data_mix` 参数（值为 mix 名，如 `mix_1t`）+ `--data_root`；默认单源保持现状。
早期计划示意（已过时，仅演进记录）：
```python
DATA_MIX_1T = {   # 早期 5 源计划稿；最终为 8 源 mix_1t，见 data_mix.py
    "dclm":        0.39,
    "fineweb_edu": 0.26,
    "stack_v2":    0.15,
    "math":        0.10,
...
- 混合 smoke：loss 正常（~ln(V) 起步、单调下降）、gnorm 正常、各源均被采样、tok/s 不退化
  （早期目标 ~250K/32GPU；**实际 64GPU+fp8 达 ~605K**，见 §10.5）。
